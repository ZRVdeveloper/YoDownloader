...
class YoFun():
    ...
    def convert(self,path):
        ...
        os.system(d1)
        os.system(d2)
        self.parent_win.info.insert('end', f" path del - ok")
        print('path del - ok')
        return path
    ...
    def download_list(self, url):
        self.parent_win.geometry("670x550")
        self.parent_win.info.insert('end', f"Завантажуємо список \n")
        v=1
        if v==1:            
            p = Playlist(url)
            self.list_size = len(p.video_urls)
            self.list_download_ok = 0
            self.parent_win.info.insert('end', f"{self.list_size} у списку \nЗавантаженно: {self.list_download_ok}\n")
